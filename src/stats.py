...
def calculate_confusion_matrix(fin_predictions: pl.DataFrame):
    ...
    metrics["Accuracy"] = (tp + tn) / (
        tp + tn + fp + fn
    )  # Ability of model to get the correct predictions
    metrics["Precision"] = tp / (
        tp + fp
    )  # Ability of model to label actual positives as positives (think retrospectively)
    metrics["Recall"] = tp / (
        tp + fn
    )  # Ability of model to correctly identify positives
...
def calculate_image_t_test(
    dataset: data.Dataset, labels: dict, ci: int = 0.95
):  # two sample t-test of unequal variance; Requires Correction of Degrees of Freedom
    """Calculate the t-test for comparing two samples of data with unequal variance."""
    assert (
        ci > 0.0 and ci < 1.0
    ), "the parameter `ci` must fall between 0 and 1.\nCurrent value: {}".format(ci)
    means = dict()
                ...
                k: torch.mean(
                    torch.Tensor(
                        [
                            torch.mean(X)
                            for X, y in dataset
                            if int(y.argmax().item()) == v
                        ]
                    )
                )
            }
        )
        standard_deviations.update(
            {
                k: torch.std(
                    torch.Tensor(
                        [
    ...
    df_denominator = (torch.square(torch.square(var1)) / ((n1**2) * (n1 - 1))) + (
        torch.square(torch.square(var2)) / ((n2**2) * (n2 - 1))
    )
    df = math.ceil(df_numerator / df_denominator)
    t = scipy.stats.t.ppf(ci, df)
    base_stats.update({"t value": t_score, "t": t, "degrees of freedom": df})
    if t_score < t:
